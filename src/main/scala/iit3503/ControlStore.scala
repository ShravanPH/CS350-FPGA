...
class ControlStore extends Module {
  ...
  // FILL ME IN!
  c03.feed.J          := 23.U
  c03.sigs.GateMARMUX := true.B
  c03.sigs.ADDR1MUX   := false.B
  c03.sigs.ADDR2MUX   := 2.U
  c03.sigs.MARMUX     := true.B
  c03.sigs.LDACV      := true.B
  c03.sigs.LDMAR      := true.B
  cStore(3)           := c03
  ...
  val c05           = 0.U.asTypeOf(new MicroInstr)
   c05.feed.J       :=  18.U
   c05.sigs.ALUK    :=  1.U
   c05.sigs.LDCC    :=  true.B
   c05.sigs.PSRMUX  :=  true.B
   c05.sigs.GateALU :=  true.B
   c05.sigs.SR1MUX  :=  1.U
   c05.sigs.LDREG   :=  true.B
  ...
  c16.feed.J     := 16.U
  c16.feed.COND  := "b001".U // wait for R
  // indirect write via read
  c16.sigs.LDMAR  := true.B
  c16.sigs.MIOEN  := true.B
  c16.sigs.RW     := true.B
  cStore(16)     := c16
  ...
  c22.feed.J        := 18.U
  c22.sigs.PCMUX    := 1.U
  c22.sigs.LDPC     := true.B
  c22.sigs.ADDR1MUX := false.B
  c22.sigs.ADDR2MUX := 2.U
  // FILL ME IN!
  cStore(22)        := c22
  ...
  c23.feed.J          := 16.U
  c23.sigs.ALUK       := 3.U
  c23.sigs.SR1MUX     := 0.U // IR[11:9]
  c23.sigs.GateALU    := true.B
  c23.sigs.LDMDR      := true.B 
  c23.feed.COND       := "b110".U
  cStore(23)          := c23
  ...
  c30.feed.J       := 32.U
  c30.sigs.LDIR    := true.B
  c30.sigs.GateMDR := true.B
  c30.sigs.MIOEN   := true.B
  c30.sigs.RW      := false.B
  // FILL ME IN!
  cStore(30)       := c30
